...
import { Text, Flex, Box, Skeleton, TooltipText, useTooltip } from '@pcmswap/uikit'
...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const basicTooltipContent =
    ifo.version === 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of PCM up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
  ...
  const unlimitedToolipContent =
    ifo.version === 3.1
      ? t(
          'For the public sale, Max PCM entry is capped by your average PCM balance in the IFO PCM pool. To increase the max entry, Stake more PCM into the IFO PCM pool',
        )
      : t(
