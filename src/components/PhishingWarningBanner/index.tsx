...
import { Text, Flex, Box, CloseIcon, IconButton, useMatchBreakpoints } from '@pcmswap/uikit'
...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("PCM is not online yet, please look forward to it!")
  // const warningText = t("please make sure you're visiting https://pcmswap.info - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/pcmswap.info)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://pcmswap.info'}
          color={text === 'https://pcmswap.info' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
