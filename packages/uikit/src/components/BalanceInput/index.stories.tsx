...
export const SiwtchUnits: React.FC = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"PCM" | "USD">("PCM");
  const conversionUnit = editingUnit === "PCM" ? "USD" : "PCM";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "PCM" ? "USD" : "PCM";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "PCM") {
      ...
      setValues({
        PCM: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
